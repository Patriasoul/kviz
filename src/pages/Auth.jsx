      ...
      <div className="patria-card overflow-hidden shadow-xl">
        <div className="patria-checker bg-primary px-7 py-8 text-primary-foreground sm:px-9 sm:py-10">
          ...
          <h1 className="mt-3 text-center font-display text-3xl font-bold sm:text-4xl">
            Sudjeluj u PatriaSoul natjecanju
          </h1>
          <p className="mx-auto mt-3 max-w-md text-center text-sm leading-relaxed text-primary-foreground/80 sm:text-base">
            Za igranje kvizova i sudjelovanje u natjecanju potrebno je imati svoj PatriaSoul račun.
            Prijavom možeš pratiti rezultat, osvajati XP, razine i bedževe te se natjecati na rang-listi.
          </p>
        </div>

        <div className="p-7 sm:p-9">
          <button onClick={onBack} className="mb-6 inline-flex items-center text-sm font-semibold text-muted-foreground hover:text-foreground">
            <ArrowLeft className="mr-2 h-4 w-4" /> Natrag
          </button>

          <p className="text-sm font-bold uppercase tracking-[.16em] text-accent">{mode === "login" ? "Dobro došao natrag" : "Pridruži se"}</p>
          <h2 className="mt-2 font-display text-2xl font-bold">{mode === "login" ? "Prijava" : "Registracija"}</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            {mode === "login" ? "Prijavi se i nastavi svoje natjecanje." : "Kreiraj račun i kreni u svoje PatriaSoul natjecanje."}
          </p>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            ...
            <button disabled={busy} className="patria-button-accent w-full justify-center disabled:cursor-not-allowed disabled:opacity-60">
              ...
              {busy ? "Molimo pričekaj…" : mode === "login" ? "Prijavi se i igraj" : "Registriraj se i igraj"}
            </button>
          </form>
