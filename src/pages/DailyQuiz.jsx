...
export default function DailyQuiz({ onBack, onComplete }) {
    ...
    return (
      <main className="mx-auto max-w-2xl px-4 py-16 text-center sm:py-24">
        <div className="patria-card p-8">
          ...
          <p className="mt-4 text-muted-foreground">Trenutačno nema dovoljno valjanih pitanja za današnji kviz. Ne prikazujemo izmišljena pitanja.</p>
    ...
    <div>
      <div className="border-b border-border bg-secondary/40">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[.16em] text-accent"><Flag className="h-4 w-4" /> Dnevni izazov</div>
          ...
          <p className="mt-4 max-w-2xl text-muted-foreground">Svi igrači tog dana dobivaju isti izbor pitanja. Novi dnevni set automatski se određuje kada započne novi dan.</p>
        </div>
      </div>
      <QuizPlayer key={dailyKey} questions={questions} title="PatriaSoul Dnevni kviz" subtitle={`Dnevni izazov · ${dailyKey}`} timeLimit={20} onComplete={onComplete} onQuit={onBack} />
    </div>
  );
}
